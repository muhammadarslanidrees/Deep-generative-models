...
def PlotData( x, g_z, data_cols, folderName, label_cols=[], seed=0, with_class=False, data_dim=2, save=False, prefix='' ):
    
    PERFORMANCE_PLOTS_DIR = '%s/perform_plots/%s'%(os.getcwd(), folderName)

    if not os.path.isdir(PERFORMANCE_PLOTS_DIR):
        os.makedirs(PERFORMANCE_PLOTS_DIR)
    ...
    if save:
        plt.savefig('%s/%s.png'%(PERFORMANCE_PLOTS_DIR, prefix.split('/')[-1]))
...
def train_discriminator_step(model_components, seed=0):
    ...
    [ cache_prefix, with_class, starting_step,
                        ...
                        show,
                        combined_loss, disc_loss_generated, disc_loss_real, xgb_losses, folderName
                        ] = model_components
...
def training_steps_WGAN(model_components):
    ...
    for i in range(starting_step, starting_step+nb_steps):
        ...
        if not i % 10: # 2x faster than testing each step...
            ...
            test_size = train.shape[0] # test using all of the actual fraud data
        ...
        if not i % log_interval:
            ...
            if show:
                PlotData( x, g_z, data_cols, folderName, label_cols, seed=0, with_class=with_class, data_dim=data_dim, 
                            save=True, prefix= data_dir + cache_prefix + '_' + str(i) )

    # save model checkpoints
    model_checkpoint_base_name = data_dir + folderName.split('/')[1] + '_{}_model_weights_step_{}.h5'
...
def adversarial_training_WGAN(arguments, train, data_cols, label_cols=[], seed=0, starting_step=0):
    ...
    [rand_dim, nb_steps, batch_size, k_d, k_g, critic_pre_train_steps,
        ...
        loss_pickle_path, show, folderName] = arguments
    ...
    model_components = [cache_prefix, with_class, starting_step, train, data_cols,
                        data_dim, label_cols, label_dim, generator_model,
                        discriminator_model, combined_model, rand_dim, nb_steps,
                        batch_size, k_d, k_g, critic_pre_train_steps, log_interval,
                        learning_rate, base_n_count, data_dir, generator_model_path,
                        discriminator_model_path, sess, _z, _x, _labels, _g_z, epsilon,
                        x_hat, gradients, _gradient_penalty, _disc_loss_generated,
                        _disc_loss_real, _disc_loss, disc_optimizer, show, combined_loss,
                        disc_loss_generated, disc_loss_real, xgb_losses, folderName]
...
def training_steps_GAN(model_components):
    ...
    [ cache_prefix, with_class, starting_step,
                        ...
                        combined_loss, disc_loss_generated, disc_loss_real, xgb_losses, folderName] = model_components  
...
#             d_l_r = discriminator_model.train_on_batch(x, classes)
    ...
    discriminator_model.save_weights(model_checkpoint_base_name.format('discriminator', i))
    pickle.dump([combined_loss, disc_loss_generated, disc_loss_real, xgb_losses], 
        open( data_dir + folderName.split('/')[1] + '_losses_step_{}.pkl'.format(i) ,'wb'))
...
def adversarial_training_GAN(arguments, train, data_cols, label_cols=[], seed=0, starting_step=0):
    ...
    [rand_dim, nb_steps, batch_size, 
             k_d, k_g, critic_pre_train_steps, log_interval, learning_rate, base_n_count,
            data_dir, generator_model_path, discriminator_model_path, loss_pickle_path, show, folderName] = arguments
